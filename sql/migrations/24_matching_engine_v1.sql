-- ============================================================
-- OMELO 24 — Matching Engine v1
--
-- Deterministic, explainable, configurable scoring (architecture/A5 §5).
--
--   * No LLM in the scoring path. Same inputs, same score.
--   * Weights come from weight_profiles, selected by job category, so a
--     weight change is a data change, never a deploy.
--   * One scorer, used symmetrically: ranking candidates for a job and
--     ranking jobs for a worker call the same function.
--   * Every score is stored with its explanation in matches.feature_vector.
--   * UNKNOWN IS NOT ZERO. A factor the worker has not filled in scores a
--     neutral 0.5 and is labelled "unknown", so an almost-empty profile can
--     still match. That is a founding constraint of the product.
-- ============================================================
...
-- ------------------------------------------------------------
-- The scorer. SECURITY DEFINER because it must read the worker's profile
-- and the job's requirements regardless of who asks. It is NOT callable by
-- clients; the public wrappers below authorise first.
-- ------------------------------------------------------------
...
begin
  ...
  -- ============================================================ GATES
  -- Gates are for facts that make hiring IMPOSSIBLE, not preferences that
  -- make it less likely.
  ...
  -- Work authorisation: only fails when we KNOW the worker is foreign to the
  -- job country, holds no right to work there, and the job will not sponsor.
  ...
  -- An EXPIRED required licence is a hard fact. A licence simply not yet
  -- added to the profile is not — most workers have no way to enter one
  -- yet — so that is scored as a gap below, never as ineligible.
  if exists (
  ...
  -- held = directly held (1.0) or via an adjacent/substitutable skill (0.5)
  with req as (
  ...
  -- trajectory_fit / company_preference (weight 0 in v1 profiles) -------------------
  ...
  -- Order explanations by how much they matter.
...
-- ------------------------------------------------------------
-- Persist a score with its explanation.
-- engine_version is part of the unique key, so recomputing under a newer
-- engine never overwrites an explanation a user was already shown.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- Score every application at submission time.
-- ------------------------------------------------------------
...
begin
  ...
  exception when others then
    -- Scoring must never block an application. A worker who can apply but
    -- has no score is a much smaller failure than a worker who cannot apply.
...
-- ============================================================
-- CLIENT-CALLABLE WRAPPERS — each authorises before scoring
-- ============================================================

-- Employer: rank every applicant for a job and store the explanations.
...
-- Worker: "why does this job match me?" — for their own work identity.
...
-- Worker: recommended jobs, scored and explained, nearest candidates first
-- so the scorer runs on a bounded set.
