-- ============================================================
-- OMELO 26 — Domain event spine (architecture/A5 §7)
--
-- One append-only outbox of business facts, written by the database in the
-- same transaction as the change. Consumers (notifications, analytics,
-- search indexing, career engine) read from here instead of each module
-- adding its own triggers. Nothing is emitted by clients.
--
-- application_events stays the per-application audit trail the worker and
-- employer see. domain_events is the platform-wide stream, named in the
...
-- Server-side only: RLS on with no policies. Platform admins read through
-- service code, never through the client API.
alter table domain_events enable row level security;
revoke all on domain_events from anon, authenticated;

-- Append-only, except marking an event processed and the ON DELETE SET NULL
-- of its foreign keys (so deleting an account or company is never blocked).
...
-- The hiring loop: translate the audit trail into business events.
...
begin
  ...
  -- OfferDeclined is emitted once, from the state change; skip the duplicate.
  if v_type is null
...
-- Trust: a verified employment is the moment a worker's identity changes.
...
-- Marketplace supply: jobs going live or closing.
...
-- Advisor fix from 25: pin the search_path.
