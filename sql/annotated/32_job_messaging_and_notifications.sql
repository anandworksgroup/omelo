-- ============================================================
-- OMELO 32 — Job-context messaging and the notification inbox
--
-- Messaging existed as tables with permissive policies:
--   * any company member (even `viewer`/`finance`) could open a
--     conversation with ANY person — unsolicited outreach with no consent;
--   * a sender could set sender_type to impersonate the other side;
--   * clients could post `action` messages with arbitrary payloads.
--
-- Now a conversation is anchored to an application (the candidate chose to
-- apply, which is the consent), is opened and written only through
-- functions, and each side is notified (candidates also by email,
-- throttled). Notifications become a real inbox: owners can read, mark read
-- and delete, but not forge or rewrite them.
-- ============================================================
...
-- Only each side's own archive flag, enforced by the guard below.
...
-- Notifications: read, mark read, delete. Never create or rewrite.
...
-- Get or open the conversation for an application. Either side may start it.
...
begin
  ...
  -- Respect a candidate who has walked away: the employer cannot keep writing.
  ...
  if not v_is_candidate then
    ...
    -- One email per conversation per 30 minutes, a few minutes after the
    -- message so a quick back-and-forth does not flood the inbox.
    ...
    -- Everyone on the hiring team who has taken part, plus whoever posted the job.
    for v_recipient in
...
-- Mark the other side's messages as read; returns how many changed.
