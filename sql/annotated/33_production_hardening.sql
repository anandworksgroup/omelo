-- ============================================================
-- OMELO 33 — Production hardening
--
-- 1. RLS performance: `auth.uid()` inside a policy is re-evaluated for
--    every row. Wrapped as `(select auth.uid())` Postgres evaluates it once
--    per statement (Supabase advisor 0003 auth_rls_initplan). Semantics are
--    identical; this rewrites every public policy that still uses the bare
--    call, so it is also correct when replayed on a fresh project.
-- 2. Every foreign key gets a supporting index (advisor 0001). Without
--    them, deleting a person or company scans whole tables and locks.
-- 3. Deployment config lives in data, not code: the Edge Function base URL
--    used by pg_cron is read from omelo_private.app_settings, so a new
--    environment sets one row instead of editing a migration.
-- ============================================================
...
-- This project's value. On a new environment:
--   update omelo_private.app_settings set value = 'https://<ref>.supabase.co/functions/v1'
