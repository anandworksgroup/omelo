...
| 57 | `57_r6_authorization_states` | Work-authorization states `employer_sponsored`, `other_authorization`, `unknown` (enum values on their own). |
| 58 | `58_r6_global_schema` | **Release 6 — Global employment & mobility, schema.** `currencies` (ISO 4217; every money column now references it) and append-only `exchange_rates` (rate, effective time, source; seeded *indicative* rates clearly labelled). `country_policies` extended into the country table (language, time zone, calling code, region; 48 countries); `locations` gain currency / language and world cities (Berlin, Munich, Amsterdam, London, Dublin, Toronto, Dubai, Sydney, Singapore, New York, Riyadh, Doha). `mobility_profiles` (self-only; authorization visibility defaults to *eligibility only*), work-authorization validity and restrictions, `license_requirements` (profession × country), `country_employment_info` (official sources, reviewed dates, not legal advice), `company_legal_entities` (multi-country employers), job sponsorship yes / no / case-by-case with support flags and remote scope (country, worldwide, time-zone range, listed countries), employment country and original pay. Employers no longer read authorization records unless the worker shares them. |
| 59 | `59_r6_global_functions` | Currency conversion with provenance, normalised pay, eligibility (eligible / potentially eligible / not eligible — only hard blockers reject; everything else is explained as missing items), the worker's pre-application check with official sources and licensing, hiring-side candidate eligibility, the six discovery tabs (near me, remote, relocation, visa sponsorship, international, work abroad), country guide, mobility RPCs, admin exchange rates; integrity triggers for jobs, legal entities, employments (country and pay currency never change) and experiences. |
| 60 | `60_r6_matcher_v12` | Matcher v1.2: the authorization gate uses eligibility (only hard blockers fail), pay compared in one currency, relocators not penalised for distance, `mobility_fit` and `eligibility_fit` factors, `eligibility` in the result. Talent search: eligibility / relocation / language / current-country filters; relocators beyond the radius; private authorization respected. Hiring-side wording never shows dates, visa types or restrictions. |
| 61 | `61_r6_sponsorship_backfill` | Aligns `visa_sponsorship` on jobs created before 58. |
| 62 | `62_r6_global_talent_search` | `omelo_search_talent` gains optional `p_filters` (global recruiter search for employers). |
| 63 | `63_r6_authorization_read_fix` | Found by the R6 test: the authorization read policy could not see the worker's (private) mobility setting, so sharing never worked — moved to a definer helper. Pre-application pay converts to the worker's home currency; on-site talent search leaves out people abroad who are not open to relocating. |
